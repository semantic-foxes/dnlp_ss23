    ...
    for i in pbar:
        for number_chosen in np.random.choice(range(size), size, replace=not is_multi_batch):
            ...
            for _ in range(weight-1):
                sample_and_train_batch(number_chosen, False, skip_optimizer_step*weight)
        
            sample_and_train_batch(number_chosen, is_optimizer_step, skip_optimizer_step*weight)
